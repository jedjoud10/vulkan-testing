...
fn main() {
    ...
    unsafe {
        ...
        // Create two buffers
        let (buffer1, mut alloc1) = device.create_buffer(4, vk::BufferUsageFlags::TRANSFER_SRC | vk::BufferUsageFlags::TRANSFER_DST, MemoryLocation::CpuToGpu, &queue);
        let ptr = alloc1.mapped_slice_mut().unwrap();
        ptr.copy_from_slice(&[1, 2, 3, 4]);
        log::debug!("{:?}", ptr);
        let (buffer2, alloc2) = device.create_buffer(4, vk::BufferUsageFlags::TRANSFER_DST | vk::BufferUsageFlags::TRANSFER_SRC, MemoryLocation::CpuToGpu, &queue);

        // Aquire a new recorder
        let mut recorder = queue.acquire(&device, false);
        let copy1 = vk::BufferCopy::builder().size(4).dst_offset(0).src_offset(0);
        let copy2 = vk::BufferCopy::builder().size(2).dst_offset(2).src_offset(0);
